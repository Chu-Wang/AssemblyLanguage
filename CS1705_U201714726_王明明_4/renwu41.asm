...
IN_VALUE DB 10;用来存储输入的要修改的信息
	DB ?
	DB 10 DUP(0)
BNAME DB 'W' XOR 'A', 'A' XOR 'A', 'N' XOR 'A', 'G' XOR 'A', 'M' XOR 'A',5 DUP(0)
COUNT1 = $-BNAME
;BPASS DB 'TEST',0,0;已存密码
BPASS DB 'T' XOR 'M','E' XOR 'M','S' XOR 'M', 'T' XOR 'M'
;COUNT2 = $-BPASS
N EQU 10;商品总数
S1  DB 'SHOP_ONE',0;商店名称
GA1 DB 'PEN',7 DUP(0),10;商品PEN
    DW 35 XOR 'B',56,70,25,68
GA2 DB 'BOOK',6 DUP(0),9;商品BOOK
    DW 12 XOR 'B',30,25,5,102
GAN DB N-3 DUP('TEMP-VALUE',8,15 XOR 'B',0,20,0,30,0,2,0,?,?)
GA3 DB 'BAG', 7 DUP(0) ,10;商品BAG
	DW 35 XOR 'B',56,70,25,102
...
START:
	MOV AX, DATA
	MOV DS,AX
		MOV ADDRTABLE,OFFSET L_IN_NAME
		MOV ADDRTABLE+2,OFFSET OVER
		MOV ADDRTABLE+4,OFFSET L_IN_PWD
		XOR AX,AX
		MOV ES,AX
		MOV AX,WORD PTR ES:[1*4]
		MOV OLD_INT1,AX
		MOV AX,WORD PTR ES:[1*4+2]
		MOV OLD_INT1+2,AX
		MOV AX,WORD PTR ES:[3*4]
		MOV OLD_INT3,AX
		MOV AX,WORD PTR ES:[3*4+2];save old int vector
		MOV OLD_INT3+2,AX
		CLI;SET NEW INT VECTOR
...
L_IN_NAME: ;用来输入用户名
	...
	JNE L_CHECK_NAME
	CMP IN_NAME+2,51H;比较是不是Q,,q--71H
	JNE L_CHECK_NAME
	JMP BX;采用间接转移抵制静态反汇编
...
OVER: 
			...
			MOV ES,AX
			CLI
			MOV AX ,WORD PTR  OLD_INT1
			MOV ES:[1*4],AX
			MOV AX,WORD PTR OLD_INT1+2
			MOV ES:[1*4+2],AX
			MOV AX,WORD PTR OLD_INT3
			MOV ES:[3*4],AX
			MOV	AX,WORD PTR OLD_INT3+2
			MOV ES:[3*4+2],AX
			STI
	MOV AH,4CH
	INT 21H
